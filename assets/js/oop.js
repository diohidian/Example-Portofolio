...
class Book {
  ...
  static setTableRow() {
    LIST_OF_BOOK.forEach((data) => {
      $("#table-book tbody").append(
        `<tr><td>${data.title}</td><td>${data.author}</td><td>${data.year}</td><td>${data.category ? data.category : "-"}</td></tr>`,
      );
    });
  }
  ...
  static find(title) {
    ...
    if (title) {
      ...
      if (book) {
        $("#table-book tbody").append(
          `<tr><td>${book.title}</td><td>${book.author}</td><td>${book.year}</td><td>${book.category ? book.category : "-"}</td></tr>`,
        );
      } else {
        $("#table-book tbody").append(
...
const biografi_2 = new BiografiBook(
  "Tarikh Khulafa",
  "Salalludin as-Suyuti",
  "1990",
);
biografi_2.setCategory("Sejarah");
LIST_OF_BOOK.push(biografi_2.getBook());

const biografi_3 = new BiografiBook("Sejarah Tuhan", "Imam Ibnu Qoyim", "1190");
biografi_3.setCategory("Sejarah");
LIST_OF_BOOK.push(biografi_1.getBook());

LIST_OF_BOOK.forEach((data) => {
  $("#table-book tbody").append(
    `<tr><td>${data.title}</td><td>${data.author}</td><td>${data.year}</td><td>${data.category ? data.category : "_"}</td></tr>`,
  );
});
